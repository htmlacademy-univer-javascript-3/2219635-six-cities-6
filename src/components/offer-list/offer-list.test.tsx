...
describe('OfferList', () => {
  it('should render the correct number of offer cards', () => {
    ...
    const {container} = render(
      <Provider store={makeStore()}>
        <MemoryRouter>
          <OfferList offers={offers} activeOfferId={null} onOfferHover={vi.fn()} />
        </MemoryRouter>
      </Provider>
    );
    expect(container.querySelectorAll('.place-card')).toHaveLength(offers.length);
  });
  ...
  it('should render empty list when no offers provided', () => {
    const {container} = render(
      <Provider store={makeStore()}>
        <MemoryRouter>
          <OfferList offers={[]} activeOfferId={null} onOfferHover={vi.fn()} />
        </MemoryRouter>
      </Provider>
    );
  ...
  it('should mark active offer card', () => {
    ...
    const {container} = render(
      <Provider store={makeStore()}>
        <MemoryRouter>
          <OfferList offers={offers} activeOfferId={activeOfferId} onOfferHover={vi.fn()} />
        </MemoryRouter>
      </Provider>
    );
