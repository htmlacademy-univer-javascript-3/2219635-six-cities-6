...
import {CITY_NAMES} from '../../mocks/cities';
...
describe('LoginPage', () => {
  ...
  it('should not dispatch login when password has no digits', () => {
    ...
    fireEvent.change(screen.getByPlaceholderText('Email'), {target: {value: 'test@test.com'}});
    fireEvent.change(screen.getByPlaceholderText('Password'), {target: {value: 'onlyletters'}});
    fireEvent.submit(screen.getByRole('button', {name: /sign in/i}).closest('form') as HTMLFormElement);
  ...
  it('should display a random city from the city list', () => {
    renderLoginPage();
    const cityLinks = CITY_NAMES.map((name) => screen.queryByText(name)).filter(Boolean);
  ...
  it('should dispatch changeCity and navigate to "/" when city link is clicked', () => {
    ...
    const cityLink = CITY_NAMES.map((name) => screen.queryByText(name)).find(Boolean) as HTMLElement;
