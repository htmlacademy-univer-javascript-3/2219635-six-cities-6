...
const makeStore = (authorizationStatus = AuthorizationStatus.NoAuth, userData: AuthInfo | null = null, favoriteOffers: ReturnType<typeof makeMockOffer>[] = []) =>
  configureStore({
    reducer: rootReducer,
    preloadedState: {
...
const renderHeader = (status = AuthorizationStatus.NoAuth, userData: AuthInfo | null = null, isActiveMain = false) =>
  render(
...
describe('Header', () => {
  ...
  it('should render favorites count when authorized', () => {
    ...
    render(
      <Provider store={makeStore(AuthorizationStatus.Auth, userData, [favoriteOffer])}>
        <MemoryRouter>
          <Header />
        </MemoryRouter>
      </Provider>
  ...
  it('should add active class to logo link when isActiveMain is true', () => {
    ...
    expect(container.querySelector('.header__logo-link')).toHaveClass('header__logo-link--active');
  });
  ...
  it('should not add active class to logo link when isActiveMain is false', () => {
    ...
    expect(container.querySelector('.header__logo-link')).not.toHaveClass('header__logo-link--active');
  });
});
